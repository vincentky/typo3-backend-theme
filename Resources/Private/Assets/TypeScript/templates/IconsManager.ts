            ...
            if (inputFile) {
                ...
                inputFile.addEventListener('change', (e: Event) => {
                    ...
                    const response = fetch(TYPO3.settings.ajaxUrls['ext-templates-upload-icon'], {
                        ...
                        .then((icon: string) => {
                            const container = inputFile.closest<HTMLDivElement>('.feature-icons-container');
                            if (container) {
                                const iconContainer = container.querySelector<HTMLSpanElement>(
                                    '.feature-icons-container-icon',
                                );
                                if (iconContainer) {
